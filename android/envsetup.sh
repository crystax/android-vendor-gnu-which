...
OPENSSL_VERSION=$(ls -1 $NDK/sources/openssl | sort | tail -n 1)
if [ -z "$OPENSSL_VERSION" ]; then
    error "Can't detect OpenSSL version in $NDK/sources/openssl!"
    return 1
fi
rsync -a $NDK/sources/openssl/$OPENSSL_VERSION/include/ $SYSROOT/usr/include/ || return 1
rsync -a $NDK/sources/openssl/$OPENSSL_VERSION/libs/$ABI/libcrypto.a $SYSROOT/$SYSROOT_LIBDIR/ || return 1
rsync -a $NDK/sources/openssl/$OPENSSL_VERSION/libs/$ABI/libssl.a $SYSROOT/$SYSROOT_LIBDIR/ || return 1
